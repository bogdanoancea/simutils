...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "simutils", "tests")); system.time(test_check("simutils", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'

...
read_csv <- function(xmlFileName, csvFileName) {
  ...
  colnames_xml <- unlist(purrr::map(xml.list, function(x) {
    x[grep("ColName", names(x))]
  }))
  ...
  types_xml <- unlist(purrr::map(xml.list, function(x) {
    x[grep("value_type", names(x), fixed = TRUE)]
  }))
  ...
  specs.dt[
    , var_coincidence1 := (grep(variable, variable2) > 0) * 1L,
    by = "row"
  ][
    , var_coincidence2 := (grep(variable2, variable) > 0) * 1L,
    by = "row"
  ][
    , var_coincidence := rowSums(.SD, na.rm = TRUE),
    .SDcols = c("var_coincidence1", "var_coincidence2")
  ]
